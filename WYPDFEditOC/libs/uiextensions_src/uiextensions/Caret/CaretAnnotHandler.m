...
-(void)onAnnotSelected:(FSAnnot*)annot
{
    ...
    if ([annot.intent isEqualToString:@"Replace"]) {
        if (annot.canModify) {
            MenuItem *styleReplaceItem = [[MenuItem alloc] initWithTitle:NSLocalizedString(@"kStyle", nil) object:self action:@selector(showReplaceStyle)];
            [array addObject:styleReplaceItem];
            _isInsert = NO;
        }
    }else{
        if (annot.canModify) {
            MenuItem *styleInsertItem = [[MenuItem alloc] initWithTitle:NSLocalizedString(@"kStyle", nil) object:self action:@selector(showInsertStyle)];
