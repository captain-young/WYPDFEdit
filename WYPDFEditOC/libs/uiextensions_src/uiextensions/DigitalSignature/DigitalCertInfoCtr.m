...
{
    float maxWidth = 0;
    CGSize size = [NSLocalizedString(@"kCertSerialNum", nil) sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(300, 64) lineBreakMode:UILineBreakModeWordWrap];
    if (maxWidth < size.width + 1) {
        maxWidth = size.width + 1;
    }
    size = [NSLocalizedString(@"kCertIssuer", nil) sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(300, 64) lineBreakMode:UILineBreakModeWordWrap];
    if (maxWidth < size.width + 1) {
        maxWidth = size.width + 1;
    }
    size = [NSLocalizedString(@"kCertStartTime", nil) sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(300, 64) lineBreakMode:UILineBreakModeWordWrap];
    if (maxWidth < size.width + 1) {
        maxWidth = size.width + 1;
    }
    size = [NSLocalizedString(@"kCertEndTime", nil) sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(300, 64) lineBreakMode:UILineBreakModeWordWrap];
    if (maxWidth < size.width + 1) {
        maxWidth = size.width + 1;
    }
    size = [NSLocalizedString(@"kCertEmail", nil) sizeWithFont:[UIFont systemFontOfSize:14] constrainedToSize:CGSizeMake(300, 64) lineBreakMode:UILineBreakModeWordWrap];
    if (maxWidth < size.width + 1) {
        maxWidth = size.width + 1;
    }
    return maxWidth;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        cell.certInfoKey.text = NSLocalizedString(@"kCertSerialNum", nil);
        cell.certInfoValue.text = self.certSerialNum;
    }else if (indexPath.row == 1){
        cell.certInfoKey.text = NSLocalizedString(@"kCertIssuer", nil);
        cell.certInfoValue.text = self.certPublisher;
    }else if (indexPath.row == 2){
        cell.certInfoKey.text = NSLocalizedString(@"kCertStartTime", nil);
        cell.certInfoValue.text = self.certStartDate;
    }else if (indexPath.row == 3){
        cell.certInfoKey.text = NSLocalizedString(@"kCertEndTime", nil);
        cell.certInfoValue.text = self.certEndDate;
    }else if (indexPath.row == 4){
        cell.certInfoKey.text = NSLocalizedString(@"kCertEmail", nil);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    sectionTitle.text = NSLocalizedString(@"kCertInfo", nil);
