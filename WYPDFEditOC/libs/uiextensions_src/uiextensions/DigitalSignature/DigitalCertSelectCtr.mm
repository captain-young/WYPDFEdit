...
- (void)viewDidLoad
{
    ...
    [self loadCertInfo];
    UIBarButtonItem *leftButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"kCancel", nil) style:UIBarButtonItemStylePlain target:self action:@selector(didCancel)];
    self.navigationItem.leftBarButtonItem = leftButton;
    UIBarButtonItem *rightButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"kOK", nil) style:UIBarButtonItemStylePlain target:self action:@selector(didOk)];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.dataArray.count < 1) {
        ...
        nonCertLabel.text = NSLocalizedString(@"kNoCert", nil);
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    if ([self.currentCertPasswordDic objectForKey:md5]) {
        ...
        certInfoCtr.title = NSLocalizedString(@"kCertInfo", nil);
...
- (void)promptForPassword:(void(^)(NSString *))complete
{
    ...
    if (self.showPromptTimes == 0) {
        alertView.title = NSLocalizedString(@"kOfflineCopyAlterTitle", nil);
    }else
    {
        alertView.title = NSLocalizedString(@"kPassWordErrorAlterTitle", nil);
    }
